...
const PageCounter = ({ pageNumber }) => {
    ...
    useEffect(() => {
        const updatedPageNumbersState = initialPageNumbersState.map(el => el.number === pageNumber ? { ...el, className: `selectedPageNumber` } : el)
    ...
    return (
        <div>
            <div>
                {pageNumbers.map(el => <h1 className={el.className} key={el.id}>{el.number}</h1>)}
            </div>
        </div>
    )
}
